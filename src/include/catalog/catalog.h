...
/**
 * The table_info class maintains metadata about a table.
 */
struct table_info {
  /**
   * Construct a new table_info instance.
   * @param schema The table schema
   * @param name The table name
   * @param table An owning pointer to the table heap
   * @param oid The unique OID for the table
   */
  table_info(Schema schema, std::string name, std::unique_ptr<TableHeap> &&table, table_oid_t oid)
...
class Catalog {
 public:
  /** Indicates that an operation returning a `table_info*` failed */
  static constexpr table_info *NULL_TABLE_INFO{nullptr};
  ...
  auto CreateTable(Transaction *txn, const std::string &table_name, const Schema &schema, bool create_table_heap = true)
    ...
    auto meta = std::make_unique<table_info>(schema, table_name, std::move(table), table_oid);
    auto *tmp = meta.get();
   ...
   * @return A (non-owning) pointer to the metadata for the table
   */
  auto GetTable(const std::string &table_name) const -> table_info * {
   ...
   * @return A (non-owning) pointer to the metadata for the table
   */
  auto GetTable(table_oid_t table_oid) const -> table_info * {
 ...
 private:
   ...
   */
  std::unordered_map<table_oid_t, std::unique_ptr<table_info>> tables_;
   ...
   */
  std::unordered_map<index_id_t, std::unique_ptr<IndexInfo>> indexes_;
