...
auto Optimizer::MatchIndex(const std::string &table_name, uint32_t index_key_idx)
  ...
  catalog_01->GetTableIndexes(table_name,indexes);
  for (auto *index_info : indexes) {
    if (key_attrs == index_info->GetIndex()->GetKeyAttrs()) {
      return std::make_optional(std::make_tuple(index_info->GetMetaData()->GetIndexId(), index_info->GetIndexName()));
    }
  }
  return std::nullopt;
}
