...
void TableGenerator::FillTable(TableInfo *info, TableInsertMeta *table_meta) {
  ...
  while (num_inserted < table_meta->num_rows_) {
    ...
    for (uint32_t i = 0; i < num_values; i++) {
      ...
      RID rid;
      bool inserted = info->GetTableHeap()->InsertTuple(Tuple(entry, info->GetSchema()), &rid, exec_ctx_->GetTransaction());
  ...
  for (auto &table_meta : insert_meta) {
    ...
    for (const auto &col_meta : table_meta.col_meta_) {
      if (col_meta.type_ != TypeId::VARCHAR) {
        Column *column = new Column(col_meta.name_, col_meta.type_, 0,0,0);
        cols.emplace_back(column);
       // delete [] column;
      } else {
        Column *column = new Column(col_meta.name_, col_meta.type_, TEST_VARLEN_SIZE,0,0,0);
        cols.emplace_back(column);
      //  delete [] column;
      }
    }
    ...
    exec_ctx_->GetCatalog()->CreateTable(table_meta.name_, &schema, exec_ctx_->GetTransaction(), table_info);
    FillTable(table_info, &table_meta);
  }
}
}  // namespace bustub
