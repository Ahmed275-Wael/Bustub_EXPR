...
auto GetMockTableSchemaOf(const std::string &table) -> Schema {
  if (table == "__mock_table_1") {
    Column *colA = new Column{"colA", TypeId::INTEGER,0,0,0};
    Column *colB = new Column{"colB", TypeId::INTEGER,0,0,0};
    return Schema{std::vector{{colA, colB}},0};
  }

  if (table == "__mock_table_2") {
    Column *colC = new Column{"colC", TypeId::VARCHAR,128,0,0,0};
    Column *colD = new Column{"colD", TypeId::VARCHAR,128,0,0,0};
    return Schema{std::vector{{colC, colD}},0};
  }

  if (table == "__mock_table_3") {
    Column *colE = new Column{"colE", TypeId::INTEGER,0,0,0};
    Column *colF = new Column{"colF", TypeId::VARCHAR,128,0,0,0};
    return Schema{std::vector{{colE, colF}},0};
  }

  if (table == "__mock_table_tas_2022") {
    Column *colG = new Column{"course_name", TypeId::VARCHAR,128,0,0,0};
    Column *colH = new Column{"TA", TypeId::VARCHAR,128,0,0,0};
    return Schema{std::vector{{colG, colH}},0};
  }

  if (table == "__mock_table_schedule_2022") {
    Column *colI = new Column{"course_id", TypeId::VARCHAR,128,0,0,0};
    Column *colJ = new Column{"classroom", TypeId::INTEGER,0,0,0};
    return Schema{std::vector{{colI, colJ}},0};
  }

  if (table == "__mock_agg_input_small" || table == "__mock_agg_input_big") {
    Column *colK = new Column{"student_id", TypeId::VARCHAR,128,0,0,0};
    Column *colL = new Column{"grade1", TypeId::INTEGER,0,0,0};
    Column *colM = new Column{"grade2", TypeId::INTEGER,0,0,0};
    Column *colN = new Column{"grade3", TypeId::INTEGER,0,0,0};
    Column *colO = new Column{"grade4", TypeId::INTEGER,0,0,0};
    Column *colP = new Column{"grade5", TypeId::INTEGER,0,0,0};
    return Schema{std::vector{{colK, colL,colM, colN,colO, colP,}},0};
  }

  if (table == "__mock_graph") {
    Column *colQ = new Column{"vertex_id", TypeId::INTEGER,0,0,0};
    Column *colR = new Column{"vertex_id2", TypeId::INTEGER,0,0,0};
    Column *colS = new Column{"vertex_id", TypeId::INTEGER,0,0,0};
    return Schema{std::vector{{colQ, colR, colS}},0};
  }

  if (table == "__mock_table_123") {
    Column *colT = new Column{"a", TypeId::INTEGER,0,0,0};

    return Schema{std::vector{colT},0};
  }

  if (table == "__mock_t1_50k" || table == "__mock_t2_100k" || table == "__mock_t3_1k" || table == "__mock_t4_1m" ||
      table == "__mock_t5_1m" || table == "__mock_t6_1m") {
        Column *colU = new Column{"c1", TypeId::INTEGER,0,0,0};
        Column *colV = new Column{"c2", TypeId::INTEGER,0,0,0};

    return Schema{std::vector{colU,colV},0};
  }

  if (table == "__mock_t7") {
    Column *colW = new Column{"c1", TypeId::INTEGER,0,0,0};
    Column *colX = new Column{"c2", TypeId::INTEGER,0,0,0};
    Column *colY = new Column{"c3", TypeId::INTEGER,0,0,0};

    return Schema{
        std::vector{colW,colX,colY},0};
  }

  if (table == "__mock_t8") {
    Column *colZ = new Column{"c1", TypeId::INTEGER,0,0,0};
    return Schema{std::vector{colZ},0};
  }
...
auto GetFunctionOf(const MockScanPlanNode *plan) -> std::function<Tuple(size_t)> {
  ...
  return [plan](size_t cursor) {
    ...
    for (auto &&Column : plan->OutputSchema().nGetColumns()) {
