...
CatalogMeta *CatalogMeta::DeserializeFrom(char *buf, MemHeap *heap) {
  ...
  buf += sizeof(uint32_t);
  BUSTUB_ASSERT(magic_num == CATALOG_METADATA_MAGIC_NUM, "Catalog_metadata does not match");
  ...
  if (init) {
    ...
    for (auto i : catalog_meta_->table_meta_pages_) {
      ...
      if (table_meta != nullptr) { // here the heap does not need to be created for the first time, it has been created when create_table last time, this time just read the information out
        ...
        BUSTUB_ASSERT(false, "Something wrong caused by deserialization, the table_meta information is null!");
      }
    ...
    for (auto i : catalog_meta_->index_meta_pages_) {
      ...
      auto *index_info = Index_Info::Create(heap_);
      Index_Metadata *index_meta = nullptr;
      Index_Metadata::DeserializeFrom(iter_page->GetData(), index_meta, index_info->GetMemHeap());
      buffer_pool_manager_->UnpinPage(i.second, false);
      if (index_meta != nullptr) {
        ...
        if (iter_table != this->catalog_meta_->table_meta_pages_.end()) {
          ...
          BUSTUB_ASSERT(false, "Something wrong, can not find the corresponding page information in the CatalogMeta!");
        }
        //
        // // 
        // block end here
        ...
        BUSTUB_ASSERT(false, "Something wrong caused by deserialization, the index_meta information is null!");
      }
    }
    }
  }
...
dberr_t CatalogManager::CreateIndex(const std::string &table_name, const string &index_name,
  ...
  if (iter_table_info == this->tables_.end()) {
    ...
    BUSTUB_ASSERT(false, "The data is inconsistent, something wrong");
  ...
  for (auto i : index_keys) {
    ...
    if ((tmp_schema->nGetColumn(tmp_index))->IsUnique()) {
      is_unique = true;
    }
  ...
  auto *index_info_tmp = Index_Info::Create(heap_);
  ...
  Index_Metadata *index_meta =
      Index_Metadata::Create(next_index_id_tmp, index_name, cor_table_id, tmp_mapping, heap_);
...
dberr_t CatalogManager::GetIndex(const std::string &table_name, const std::string &index_name,
  ...
  // this index is found, find the corresponding Index_Info pointer
...
dberr_t CatalogManager::GetTableIndexes(const std::string &table_name, std::vector<Index_Info *> &indexes) const {
  ...
  // this function will put all the Index_Info information of the table_name into the vector
...
dberr_t CatalogManager::DropIndex(const string &table_name, const string &index_name, bool refresh) {
  ...
  Index_Info *IFO = iter_indexIFO->second; // the Index_Info of the index;

  // 3. try to clear the B_plus tree.
  if (IFO->GetIndex()->Destroy() == 0) {
  ...
  // clear the Index_Info mapping away
...
dberr_t CatalogManager::LoadTable(const table_id_t table_id, const page_id_t page_id) {
    BUSTUB_ASSERT(false, "Not Implemented yet"); // Not used in my implementation -> directly implement it in the ctor of the class
}
