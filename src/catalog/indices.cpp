...
using namespace std;
namespace bustub{
Index_Metadata *Index_Metadata::Create(const index_id_t index_id, const string &index_name,
                                     const table_id_t table_id, const vector<uint32_t> &key_map,
                                     MemHeap *heap) {
  void *buf = heap->Allocate(sizeof(Index_Metadata));
  return new(buf)Index_Metadata(index_id, index_name, table_id, key_map);
}

uint32_t Index_Metadata::SerializeTo(char *buf) const {
  uint32_t ofs = 0;
...
uint32_t Index_Metadata::GetSerializedSize() const {
...
uint32_t Index_Metadata::DeserializeFrom(char *buf, Index_Metadata *&index_meta, MemHeap *heap) {
  ...
  BUSTUB_ASSERT(magic_num == INDEX_METADATA_MAGIC_NUM, "INDEX_METADATA_MAGIC_NUM does not match");
  buf += sizeof(uint32_t);
