...
auto Planner::PlanColumnRef(const BoundColumnRef &expr, const std::vector<AbstractPlanNodeRef> &children)
  ...
  if (children.size() == 1) {
    ...
    bool found = false;
    for (const auto &col : schema.nGetColumns()) {
    ...
    auto col_type = schema.nGetColumn(col_idx)->GetType();
  ...
  if (children.size() == 2) {
    ...
    if (col_idx_left) {
      auto col_type = left_schema.nGetColumn(*col_idx_left)->GetType();
      return std::make_tuple(col_name, std::make_shared<ColumnValueExpression>(0, *col_idx_left, col_type));
    }
    if (col_idx_right) {
      auto col_type = right_schema.nGetColumn(*col_idx_right)->GetType();
