...
auto Planner::PlanInsert(const InsertStatement &statement) -> AbstractPlanNodeRef {
  ...
  auto &table_schema = statement.table_->schema_.nGetColumns();
  auto &child_schema = select->OutputSchema().nGetColumns();
  ...
  auto insert_schema = std::make_shared<Schema>(std::vector{Column("__bustub_internal.insert_rows", TypeId::INTEGER,0,0,0)});
...
auto Planner::PlanDelete(const DeleteStatement &statement) -> AbstractPlanNodeRef {
  ...
  Column * delete_rows = new Column("__bustub_internal.delete_rows", TypeId::INTEGER,0,0,0);
  auto delete_schema = std::make_shared<Schema>(std::vector{delete_rows},0);
...
auto Planner::PlanUpdate(const UpdateStatement &statement) -> AbstractPlanNodeRef {
  ...
  for (size_t idx = 0; idx < target_exprs.size(); idx++) {
    if (target_exprs[idx] == nullptr) {
      target_exprs[idx] =
          std::make_shared<ColumnValueExpression>(0, idx, filter->output_schema_->nGetColumn(idx)->GetType());
    }
  }
  Column * update_rows = new Column("__bustub_internal.update_rows", TypeId::INTEGER,0,0,0);
  auto update_schema = std::make_shared<Schema>(std::vector{update_rows},0);
