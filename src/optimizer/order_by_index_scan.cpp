...
auto Optimizer::OptimizeOrderByAsIndexScan(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  if (optimized_plan->GetType() == PlanType::Sort) {
    ...
    if (child_plan->GetType() == PlanType::SeqScan) {
      ...
      catalog_01->GetTable(seq_scan.table_name_,table_info);
      std::vector<Index_Info *> indices;
      catalog_01->GetTableIndexes(seq_scan.table_name_,indices); 

      for (auto *index : indices) {
        auto &columns = index->GetIndexKeySchema()->nGetColumns();
        if (columns.size() == 1 &&
            columns[0]->GetName() == table_info->GetSchema()->nGetColumn(order_by_column_id)->GetName()) {
          // Index matched, return index scan instead
          return std::make_shared<IndexScanPlanNode>(optimized_plan->output_schema_, index->GetMetaData()->GetIndexId());
        }
      }
    }
  }
