...
TableHeap::TableHeap(BufferPoolManager *buffer_pool_manager, LockManager *lock_manager, LogManager *log_manager, Schema *schema,
...
TableHeap::TableHeap(BufferPoolManager *buffer_pool_manager, LockManager *lock_manager, LogManager *log_manager, Schema *schema,
                     Transaction *txn)
    : buffer_pool_manager_(buffer_pool_manager), lock_manager_(lock_manager), log_manager_(log_manager), schema_(schema) {
...
auto TableHeap::InsertTuple(const Tuple &tuple, RID *rid, Transaction *txn) -> bool {
  if (tuple.size_ + 32 > BUSTUB_PAGE_SIZE) {  // larger than one page size
    ...
    LOG_DEBUG("MOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOre");
    return false;
  }
  ...
  if (cur_page == nullptr) {
    txn->SetState(TransactionState::ABORTED);
    LOG_DEBUG("feeeeetchhhhhhhhhhhh");
    return false;
  }
...
void TableHeap::FreeHeap() {
  ...
  for (page_id_t i = GetFirstPageId(); i != INVALID_PAGE_ID; i = next) {
    ...
    if (state == false) {
      std::cerr << "TableHeap::FreeHeap Failed";
    }
  }
  // Free All the Schema
  std::vector<Column *> columns = schema_->nGetColumns();
