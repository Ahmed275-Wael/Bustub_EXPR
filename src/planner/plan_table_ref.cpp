...
auto Planner::PlanSubquery(const BoundSubqueryRef &table_ref, const std::string &alias) -> AbstractPlanNodeRef {
  ...
  for (const auto &col : select_node->OutputSchema().nGetColumns()) {
...
auto Planner::PlanBaseTableRef(const BoundBaseTableRef &table_ref) -> AbstractPlanNodeRef {
  ...
  // ProjectionExecutor.
  auto &catalog_01 = catalog_;
  TableInfo *table = nullptr;
  catalog_01->GetTable(table_ref.table_,table);
  BUSTUB_ASSERT(table, "table not found");

  if (StringUtil::StartsWith(table->GetTableName(), "__")) {
    // Plan as MockScanExecutor if it is a mock table.
    if (StringUtil::StartsWith(table->GetTableName(), "__mock")) {
      return std::make_shared<MockScanPlanNode>(std::make_shared<Schema>(SeqScanPlanNode::InferScanSchema(table_ref)),
                                                table->GetTableName());
    }
    throw bustub::Exception(fmt::format("unsupported internal table: {}", table->GetTableName()));
  }
...
auto Planner::PlanExpressionListRef(const BoundExpressionListRef &table_ref) -> AbstractPlanNodeRef {
  ...
  for (const auto &col : first_row) {
    ...
    if (col->GetReturnType() != TypeId::VARCHAR) {
      Column * col1 = new Column(col_name, col->GetReturnType(),0,0,0);
      cols.emplace_back(col1);
    } else {
      Column *col1 = new Column(col_name, col->GetReturnType(), VARCHAR_DEFAULT_LENGTH,0,0,0);
      cols.emplace_back(col1);
    }
    idx += 1;
  }
  auto schema = std::make_shared<Schema>(cols,0);
